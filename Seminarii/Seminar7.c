...
int calculHash(int cod, const char* nume, int dimensiune) {
	//suma codurilor ascii din nume impartit la cod - de obicei sa se creeze fct codului has a.i sa fie unica (la test)
	//lungime nume cu cod:
	if (dimensiune > 0) {
		int rez = cod * strlen(nume); //-e mai mare ca 5=>iese din spatiu=>ne-ar trb un modulo pt a se incadra in spatiul de 5 al tabelei
...
void inserareGalerieInTabela(HashTable hash, Galerie galerie) {
	if (hash.dim > 0) {
		...
		if (hash.vector[pozitie] != NULL) {//acest if este aici doar pt a prezenta situatia de coliziune
			inserareLaFinal(&(hash.vector[pozitie]), galerie);//am coliziune
		}
		else {
			inserareLaFinal(&(hash.vector[pozitie]), galerie);//nu am coliziune
		}
	}
}
